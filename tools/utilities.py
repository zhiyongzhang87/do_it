...
class LogHandler(object):
    def __init__(self, arg_id: str):
        self._time_format = '%Y-%m-%dT%H:%M:%S.%f'
        self._id = arg_id
        self._log_dir = os.path.join(os.getcwd(), 'log')
        t_timestamp = datetime.datetime.now()
        self._log_full_path = os.path.join(self._log_dir, F'{self._id}_{t_timestamp.strftime("%Y%m%d_%H%M%S")}.log')

        self._lock = threading.Lock()
        self._retry_limit = 5

        try:
            if(os.path.exists(self._log_dir) == False):
                os.mkdir(self._log_dir)
            self._log_handle = open(self._log_full_path, 'w')
            self.log_message(F'{self._id} starts.')
        except (OSError, IOError) as e:
            raise ValueError('Error: Cannot open log file %s.' % (self._log_full_path))
    ...
    def log_message(self, arg_message: str) -> None:
        t_timestamp = datetime.datetime.now()
        t_contents = F'{t_timestamp.strftime(self._time_format)}: {arg_message}'
        self._write_log_to_file(t_contents)

    def log_warning(self, arg_message: str) -> None:
        t_timestamp = datetime.datetime.now()
        t_contents = F'{t_timestamp.strftime(self._time_format)}: **WARNING** {arg_message}'
        self._write_log_to_file(t_contents)

    def log_error(self, arg_message: str) -> None:
        t_timestamp = datetime.datetime.now()
        t_contents = F'{t_timestamp.strftime(self._time_format)}: {"*" * 10}ERROR{"*" * 10}\n\t{arg_message}'
        self._write_log_to_file(t_contents)
